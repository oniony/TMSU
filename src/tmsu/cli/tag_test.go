...
func TestSingleTag(test *testing.T) {
	...
	if err := TagCommand.Exec(store, Options{}, []string{"/tmp/tmsu/a", "apple"}); err != nil {
		test.Fatal(err)
	}
...
func TestMultipleTags(test *testing.T) {
	...
	if err := TagCommand.Exec(store, Options{}, []string{"/tmp/tmsu/a", "apple", "banana", "clementine"}); err != nil {
		test.Fatal(err)
	}
...
func TestTagMultipleFiles(test *testing.T) {
	...
	if err := TagCommand.Exec(store, Options{Option{"--tags", "-t", "", true, "apple banana"}}, []string{"/tmp/tmsu/a", "/tmp/tmsu/b"}); err != nil {
		test.Fatal(err)
	}
